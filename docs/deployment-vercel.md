...
5. Verify `/`, `/intelligence/healthz`, `/intelligence/v3/model`, and a `/intelligence/v3/demand` request.
...
The command center first attempts the full Go operations API. If it is unavailable, the server-rendered live-fusion path loads NASA EONET, NOAA/NWS, and USGS concurrently. The six highest-priority signals are sent to the Python service, which loads Open-Meteo and NASA POWER concurrently before neural inference. If environmental enrichment exceeds the response budget, the web service retries inference with explicitly unavailable context rather than silently replacing the neural result.
