...
def allocate(
    incident: Incident, resources: list[Resource], limit: int = 3
    ...
    for suitability, resource, distance, matches in candidates[:limit]:
        ...
        allocation_key = f"{incident.id}:{resource.id}:{created[:16]}"
        allocation_id = "alloc-" + uuid.uuid5(uuid.NAMESPACE_URL, allocation_key).hex[:12]
